...
class BlankReader:
    def __init__(self, info_file='dump/data.json', model='model/AM_50.model') -> None:
    ...
    def _set_blank(self, path):
        ...
        self._cur_canvas = cv2.imread(f'{path}.png')

    def _save_blank(self, path):
        cv2.imwrite(f'{path}_processed.png', self._cur_canvas)

    def _recover_blank(self):
        _, data, points, _ = cv2.QRCodeDetector().detectAndDecodeMulti(self._cur_canvas)
        unrecognized = False
        ...
        self._cur_canvas = cv2.warpAffine(src=self._cur_canvas,
                                      M=cv2.getAffineTransform(src=np.float32([self._cur_coords['QR']['bot_left'][1],
                                                                               self._cur_coords['QR']['top_left'][1],
                                                                               self._cur_coords['QR']['top_right'][1]]),
    ...
    def _find_boxes(self):
        for row_key, row in self._ref_coords['Fields'].items():
            ...
            for box_key, box in row.items():
                ...
                x2 = box[1][0] + delta
                y2 = box[1][1] + delta
                roi = self._cur_canvas[y1:y2, x1:x2]
